...
class Seon_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = seon()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'seon_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'seon_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->seon_generate_file_options();
		} );
	}

	function seon_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->seon_generate_file_options();
			$this->seon_generate_file();
		}
	}

    function seon_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->seon_options_output() )
        ) );
    }

	function seon_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function seon_options_output() {
		$theme_colors                    = seon_configs('theme_colors');
        $links                           = seon_configs('link');
        $gradients                       = seon_configs('gradient');
		ob_start();

		printf('$seon_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$seon_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
    ...
    /* Inline CSS */
    function seon_enqueue() {
        $css = $this->seon_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function seon_inline_css() {
        ob_start();
