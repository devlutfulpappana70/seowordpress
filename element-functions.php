 ...
 * Swipper Lib
*/
if(!function_exists('seon_elements_scripts')){
    add_action( 'wp_enqueue_scripts', 'seon_elements_scripts');
    function seon_elements_scripts() {  
        ...
        wp_register_script('seon-particle', get_template_directory_uri() . '/elements/widgets/js/particle.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_register_script('seon-parallax', get_template_directory_uri() . '/elements/widgets/js/parallax.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        ...
        wp_register_script('seon-counter', get_template_directory_uri() . '/elements/widgets/js/counter.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_register_script('seon-accordion', get_template_directory_uri() . '/elements/widgets/js/accordion.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_register_script('seon-tabs', get_template_directory_uri() . '/elements/widgets/js/tabs.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_register_script('seon-progressbar', get_template_directory_uri() . '/elements/widgets/js/progressbar.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_register_script('seon-countdown', get_template_directory_uri() . '/elements/widgets/js/countdown.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        ...
        wp_register_script('seon-pie-chart', get_template_directory_uri() . '/elements/widgets/js/pie-chart.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_enqueue_script('seon-elementor', get_template_directory_uri() . '/elements/widgets/js/elementor.js', [ 'jquery' ], $theme->get( 'Version' ), true);
    }
}
...
*/
if(!function_exists('seon_register_custom_icon_library')){
    add_filter('elementor/icons_manager/native', 'seon_register_custom_icon_library');
    function seon_register_custom_icon_library($tabs){
        $custom_tabs = [
            'pxl_icon1' => [
                'name' => 'flaticon',
                'label' => esc_html__( 'Seon', 'seon' ),
                'url' => false,
                'enqueue' => false,
                'prefix' => 'flaticon-',
...
*/
if(!function_exists('seon_get_class_widget_path')){
    function seon_get_class_widget_path(){
...
*/
function seon_get_post_type_options($pt_supports=[]){
...
*/
function seon_get_post_grid_layout($pt_supports = []){
    $post_types  = seon_get_post_type_options($pt_supports); 
    $result = [];
    if (!is_array($post_types))
        return $result;
    foreach ($post_types as $name => $label) {
        $result[] = array(
            'name'     => 'layout_'.$name,
            'label'    => sprintf(esc_html__( 'Select Template of %s', 'seon' ), $label),
            'type'     => 'layoutcontrol',
            'default' => 'post-1',
            'options'  => seon_get_grid_layout_options($name),
...
function seon_get_grid_layout_options($posttype_name){
    $option_layouts = [];
...
function seon_get_grid_term_by_posttype($pt_supports = [], $args=[]){
    ...
    foreach ($post_types as $name => $label) {
        ...
        $result[] = array(
            'name'     => 'source_'.$name,
            'label'    => sprintf(esc_html__( 'Select Term of %s', 'seon' ), $label),
...
function seon_get_grid_ids_by_posttype($pt_supports = [], $args = []){
    $args = wp_parse_args($args, ['condition' => 'post_type', 'custom_condition' => []]);
    $post_types = seon_get_post_type_options($pt_supports);
    $result = [];
    if (!is_array($post_types))
    ...
    foreach ($post_types as $name => $label) {
        ...
        $posts = seon_list_post($name, false);
 
        $result[] = array(
            'name' => 'source_' . $name . '_post_ids',
            'label' => sprintf(esc_html__('Select posts', 'seon'), $label),
            'type'     => \Elementor\Controls_Manager::SELECT2,
            'multiple' => true,
            'options' => $posts,
            'condition' => array_merge(
...
*/
function seon_get_post_carousel_layout($pt_supports = []){
    $post_types  = seon_get_post_type_options($pt_supports); 
    $result = [];
    if (!is_array($post_types))
        return $result;
    foreach ($post_types as $name => $label) {
        $result[] = array(
            ...
            'type'     => 'layoutcontrol',
            'default' => 'post-1',
            'options'  => seon_get_carousel_layout_options($name),
...
function seon_get_carousel_layout_options($posttype_name){
    $option_layouts = [];
    switch ($posttype_name) {
        case 'portfolio':
            $option_layouts = [
...
function seon_get_carousel_term_by_posttype($pt_supports = [], $args=[]){
    $args = wp_parse_args($args, ['condition' => 'post_type', 'custom_condition' => []]);
    $post_types  = seon_get_post_type_options($pt_supports); 
    $result = [];
    if (!is_array($post_types))
...
*/
function seon_get_post_modern_layout($pt_supports = []){
    $post_types  = seon_get_post_type_options($pt_supports); 
    $result = [];
    if (!is_array($post_types))
        return $result;
    foreach ($post_types as $name => $label) {
        $result[] = array(
            ...
            'type'     => 'layoutcontrol',
            'default' => 'post-1',
            'options'  => seon_get_modern_layout_options($name),
            'prefix_class' => 'post-layout-',
            'condition' => [
                'post_type' => [$name]
            ]
...
function seon_get_modern_layout_options($posttype_name){
    $option_layouts = [];
    switch ($posttype_name) {
...
*/
function seon_get_post_slip_layout($pt_supports = []){
    $post_types  = seon_get_post_type_options($pt_supports); 
    $result = [];
    if (!is_array($post_types))
        return $result;
    foreach ($post_types as $name => $label) {
        $result[] = array(
            ...
            'type'     => 'layoutcontrol',
            'default' => 'portfolio-1',
            'options'  => seon_get_slip_layout_options($name),
...
function seon_get_slip_layout_options($posttype_name){
    $option_layouts = [];
    switch ($posttype_name) {
...
/* Icon render */ 
function seon_elementor_icon_render( $settings, $args = []){
...
function seon_widget_animate_v2() {
    $seon_animate_v2 = array(
        '' => 'None',
        'wow bounce' => 'bounce',
        'wow flash' => 'flash',
        'wow pulse' => 'pulse',
    ...
    return $seon_animate_v2;
}

/**
 * Pagram Animation
*/
if(!function_exists('seon_widget_animation_settings')){
    function seon_widget_animation_settings($args = []){
        $args = wp_parse_args($args, [
        ...
        return array(
            ...
            'label'     => esc_html__('Animation', 'seon'),
            'tab'       => $args['tab'],
...
if(!function_exists('seon_widget_color_type')){
    function seon_widget_color_type($args = []){
        ...
        $options = array(
            ...
            array(
                ...
                'label' => esc_html__('Normal Color', 'seon' ),
            ...
            array(
                ...
                'label' => $args['label'] .' '.esc_html__('Gradient Color', 'seon' ),
            ...
            array(
                ...
                'label'       => ucfirst( str_replace('_', '', $args['prefix']) ).' '. esc_html__( 'Start Popover', 'seon' ),
                'type'        => 'pxl_start_popover',
                'condition'   => $args['condition'],
            ),
            array(
                'name' => $args['prefix'].'_gradient_color_from',
                'label' => esc_html__('From', 'seon' ),
                'type' => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
            ...
            array(
                ...
                'label'       => ucfirst( str_replace('_', '', $args['prefix']) ).' '. esc_html__( 'End Popover', 'seon' ),
...
if(!function_exists('seon_widget_gradient_color')){
    function seon_widget_gradient_color($args = []){
        $gradient_prefix_class = 'pxl-';
        $gradient_return_value = 'gradient';
        $args = wp_parse_args($args, [
            'label' => '',
...
if(!function_exists('seon_widget_gradient_color_rotate')){
    function seon_widget_gradient_color_rotate($args = []){
        $gradient_prefix_class = 'pxl-';
        $gradient_return_value = 'gradient';
        $args = wp_parse_args($args, [
            'label' => '',
