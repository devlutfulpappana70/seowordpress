 ...
 * @package Case-Themes
 */
add_action('after_setup_theme', 'seon_setup');
function seon_setup(){

    //Set the content width in pixels, based on the theme's design and stylesheet.
    $GLOBALS['content_width'] = apply_filters( 'seon_content_width', 1200 );

    // Make theme available for translation.
    load_theme_textdomain( 'seon', get_template_directory() . '/languages' );
    ...
    register_nav_menus( array(
        'primary' => esc_html__( 'Primary', 'seon' ),
    ) );
    ...
    // Enable support for Post Thumbnails on posts and pages.
    add_theme_support('post-thumbnails');
    add_image_size( 'seon-thumb-xs', 120, 104, true );
    add_image_size( 'seon-portfolio', 600, 600, true );
 ...
 */
add_action( 'widgets_init', 'seon_widgets_position' );
function seon_widgets_position() {
	register_sidebar( array(
		'name'          => esc_html__( 'Blog Sidebar', 'seon' ),
		'id'            => 'sidebar-blog',
	...
	if (class_exists('ReduxFramework')) {
		register_sidebar( array(
			'name'          => esc_html__( 'Page Sidebar', 'seon' ),
			'id'            => 'sidebar-page',
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title"><span>',
	...
	if ( class_exists( 'Woocommerce' ) ) {
		register_sidebar( array(
			'name'          => esc_html__( 'Shop Sidebar', 'seon' ),
			'id'            => 'sidebar-shop',
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title"><span>',
 ...
 */
add_action( 'wp_enqueue_scripts', 'seon_scripts' );
function seon_scripts() {  
    $seon_version = wp_get_theme( get_template() );
    ...
    /* Icons Lib - CSS */
    wp_enqueue_style('flaticon', get_template_directory_uri() . '/assets/fonts/flaticon/css/flaticon.css' , array(), $seon_version->get( 'Version' ));
    ...
    /* Parallax Scroll */
    wp_enqueue_script( 'pxl-parallax-background', get_template_directory_uri() . '/assets/js/libs/parallax-background.js', array( 'jquery' ), $seon_version->get( 'Version' ), true );
    wp_enqueue_script( 'pxl-parallax-scroll', get_template_directory_uri() . '/assets/js/libs/parallax-scroll.js', array( 'jquery' ), $seon_version->get( 'Version' ), true );
    ...
    /* Woocommerce */
    wp_enqueue_script( 'pxl-woocommerce', get_template_directory_uri() . '/woocommerce/js/woocommerce.js', array( 'jquery' ), $seon_version->get( 'Version' ), true );

    /* Cookie */
    wp_register_script( 'pxl-cookie', get_template_directory_uri() . '/assets/js/libs/cookie.js', array( 'jquery' ), '1.4.1', true );

    /* Smooth Scroll */
    $smooth_scroll = seon()->get_theme_opt( 'smooth_scroll', 'off' );
    if($smooth_scroll == 'on') {
	...
	wp_enqueue_style( 'pxl-caseicon', get_template_directory_uri() . '/assets/css/caseicon.css', array(), $seon_version->get( 'Version' ) );
    wp_enqueue_style( 'pxl-grid', get_template_directory_uri() . '/assets/css/grid.css', array(), $seon_version->get( 'Version' ) );
	wp_enqueue_style( 'pxl-style', get_template_directory_uri() . '/assets/css/style.css', array(), $seon_version->get( 'Version' ) );
	wp_add_inline_style( 'pxl-style', seon_inline_styles() );
    wp_enqueue_style( 'pxl-base', get_template_directory_uri() . '/style.css', array(), $seon_version->get( 'Version' ) );
    wp_enqueue_style( 'pxl-google-fonts', seon_fonts_url(), array(), null );
	wp_enqueue_script( 'pxl-main', get_template_directory_uri() . '/assets/js/theme.js', array( 'jquery' ), $seon_version->get( 'Version' ), true );
    wp_localize_script( 'pxl-main', 'main_data', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
    if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
        wp_enqueue_script( 'comment-reply' );
    }
    do_action( 'seon_scripts');
}

/**
 * Enqueue Styles Scripts : Back-End
 */
add_action('admin_enqueue_scripts', 'seon_admin_style');
function seon_admin_style() {
    $theme = wp_get_theme( get_template() );
    wp_enqueue_style( 'seon-admin-style', get_template_directory_uri() . '/assets/css/admin.css', array(), $theme->get( 'Version' ) );
...
add_action( 'elementor/editor/before_enqueue_scripts', function() {
    ...
    wp_enqueue_style( 'seon-admin-style', get_template_directory_uri() . '/assets/css/admin.css');
} );

/* Favicon */
add_action('wp_head', 'seon_site_favicon');
function seon_site_favicon(){
    $favicon = seon()->get_theme_opt( 'favicon' );
 ...
 */
add_action( 'wp_head', 'seon_pingback_header' );
function seon_pingback_header(){
...
/* Hidden Panel */
add_action( 'pxl_anchor_target', 'seon_hook_anchor_templates_hidden_panel');
function seon_hook_anchor_templates_hidden_panel(){

    $hidden_templates = seon_get_templates_slug('hidden-panel');
...
if(!function_exists('seon_hook_anchor_hidden_panel')){
    function seon_hook_anchor_hidden_panel($args){
...
/* Elementor Popup */
add_action( 'pxl_anchor_target', 'seon_hook_anchor_templates_popup');
function seon_hook_anchor_templates_popup(){

    $popup_templates = seon_get_templates_slug('popup');
...
if(!function_exists('seon_hook_anchor_popup')){
    function seon_hook_anchor_popup($args){ ?>
...
/* Page Popup */
add_action( 'pxl_anchor_target', 'seon_hook_anchor_templates_page_popup');
function seon_hook_anchor_templates_page_popup(){

    $page_templates = seon_get_templates_slug('page');
...
if(!function_exists('seon_hook_anchor_page_popup')){
    function seon_hook_anchor_page_popup($args){ ?>
...
/* Cart Sidebar */
if(!function_exists('seon_hook_anchor_cart')){
    function seon_hook_anchor_cart(){
        if(class_exists('Woocommerce')) :
            global $woocommerce; ?>
