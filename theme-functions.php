 ...
 * Google Fonts
*/
function seon_fonts_url() {
    $fonts_url = '';
    $fonts     = array();
    $subsets   = 'latin,latin-ext';

    if ( 'off' !== _x( 'on', 'Roboto font: on or off', 'seon' ) ) {
        $fonts[] = 'Roboto:ital,wght@0,400;0,500;0,600;0,700';
    }

    if ( 'off' !== _x( 'on', 'Inter Tight font: on or off', 'seon' ) ) {
        $fonts[] = 'Inter Tight:ital,wght@0,400;0,500;0,600;0,700;0,800;0,900;1,400;1,500;1,600;1,700;1,800;1,900';
    }
 ...
 * Get page ID by Slug
*/
function seon_get_id_by_slug($slug, $post_type){
 ...
 **/
function seon_content_by_slug($slug, $post_type){
    $content = seon_get_content_by_slug($slug, $post_type);

    $id = seon_get_id_by_slug($slug, $post_type);
 ...
 * Get content by slug
 **/
function seon_get_content_by_slug($slug, $post_type){
    $content = get_posts(
        array(
            'name'      => $slug,
            'post_type' => $post_type
 ...
 * Custom Comment List
 */
function seon_comment_list( $comment, $args, $depth ) {
	if ( 'div' === $args['style'] ) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
 ...
 * Paginate Links
 */
function seon_ajax_paginate_links($link){
 ...
 * Image Size Crop
 */
if(!function_exists('seon_get_image_by_size')){
    function seon_get_image_by_size( $params = array() ) {
 ...
 * Search Form
 */
function seon_header_mobile_search_form() { 
    $search_mobile = seon()->get_theme_opt( 'search_mobile', false );
    $search_placeholder_mobile = seon()->get_theme_opt( 'search_placeholder_mobile' );
    if($search_mobile) : ?>
    <div class="pxl-header-mobile-search pxl-hide-xl">
        <form role="search" method="get" action="<?php echo esc_url(home_url( '/' )); ?>">
            <input type="text" placeholder="<?php if(!empty($search_placeholder_mobile)) { echo esc_attr($search_placeholder_mobile); } else { esc_attr_e('Search...', 'seon'); } ?>" name="s" class="search-field" />
...
if(function_exists( 'pxl_register_shortcode' )) {
    function seon_year_shortcode() {
        ob_start(); ?>
            <span><?php the_date('Y'); ?></span>
        <?php $output = ob_get_clean();
        return $output;
    }
    pxl_register_shortcode('pxl_year', 'seon_year_shortcode');
}

/* Highlight Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function seon_text_highlight_shortcode( $atts = array() ) {
    ...
    pxl_register_shortcode('highlight', 'seon_text_highlight_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function seon_image_highlight_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'id_image' => '',
        ), $atts));
    ...
    pxl_register_shortcode('highlight_image', 'seon_image_highlight_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function seon_text_highlight_shortcode_editor( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_highlight', 'seon_text_highlight_shortcode_editor');
}

/* Typewriter Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function seon_text_typewriter_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
        ), $atts));
    ...
    pxl_register_shortcode('typewriter', 'seon_text_typewriter_shortcode');
}

/* Button Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function seon_btn_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
         'style' => 'btn-slider1',
         'icon_class' => '',
    ...
    pxl_register_shortcode('pxl_button', 'seon_btn_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function seon_btn_submit_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
    ...
    pxl_register_shortcode('pxl_button_submit', 'seon_btn_submit_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function seon_slider_arrow( $atts = array() ) {
        extract(shortcode_atts(array(
         'type' => 'next',
         'style' => 'style-1',
        ), $atts));
    ...
    pxl_register_shortcode('slider_arrow', 'seon_slider_arrow');
}

// Shortcode Row/Column Grid
if(function_exists( 'pxl_register_shortcode' )) {
    function seon_start_row_shortcode( $atts = array() ) {
        ob_start(); ?>
    ...
    pxl_register_shortcode('pxl_start_row', 'seon_start_row_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function seon_end_row_shortcode( $atts = array() ) {
        ob_start(); ?>
            </div>
        </div>       
        <?php $output = ob_get_clean();
    ...
    pxl_register_shortcode('pxl_end_row', 'seon_end_row_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function seon_start_col_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'class' => 'col-12',
        ), $atts));
        ob_start(); ?>
    ...
    pxl_register_shortcode('pxl_start_column', 'seon_start_col_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function seon_end_col_shortcode( $atts = array() ) {
        ob_start(); ?>
        </div>  
        <?php $output = ob_get_clean();
        return $output;
    }
    pxl_register_shortcode('pxl_end_column', 'seon_end_col_shortcode');
}
...
/* Gallery Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function seon_gallery_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'link_video' => '',
         'images_id' => '',
         'col' => '2',
        ...
        foreach ($pxl_images as $key => $img_id) :
            ...
            <div class="pxl--item">
                <div class="pxl--item-inner">
                    <a href="<?php echo esc_url($thumbnail_url); ?>" data-elementor-lightbox-slideshow="pxl-gallery-<?php echo esc_attr($pxl_g_id); ?>"><?php echo seon_html($thumbnail); ?></a>
    ...
    pxl_register_shortcode('pxl_gallery', 'seon_gallery_shortcode');
}

/* Addd shortcode Video button */
if(function_exists( 'pxl_register_shortcode' )) {
    function seon_video_button_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'link' => '',
         'text' => '',
         'class' => 'style1',
    ...
    pxl_register_shortcode('pxl_video_button', 'seon_video_button_shortcode');
}

/* Get Category Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function seon_post_category_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'items' => '6',
         'columns' => '2',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_post_category', 'seon_post_category_shortcode');
}

/* Slider 1  */
if(function_exists( 'pxl_register_shortcode' )) {
    function seon_slider_price_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'price' => '',
         'desc' => '',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_slider_price', 'seon_slider_price_shortcode');
}

//**
 /* Custom Widget Categories
 */
add_filter('wp_list_categories', 'seon_wg_category_count');
function seon_wg_category_count($output) {
 ...
 */
add_filter('get_archives_link', 'seon_archive_count_span');
function seon_archive_count_span($links) {
    $dir = is_rtl() ? 'pxl-left' : 'pxl-right';
 ...
 */
add_filter('wp_list_categories', 'seon_wc_cat_count_span');
function seon_wc_cat_count_span($links) {
    $dir = is_rtl() ? 'pxl-left' : 'pxl-right';
 ...
 */
function seon_get_mega_menu_builder_id(){
    $mn_id = [];
 ...
 */
function seon_get_page_popup_builder_id(){
    $pp_id = [];
...
function seon_mouse_move_animation() { 
    $mouse_move_animation = seon()->get_theme_opt('mouse_move_animation', false); 
    if($mouse_move_animation) {
        wp_enqueue_script( 'seon-cursor', get_template_directory_uri() . '/assets/js/libs/cursor.js', array( 'jquery' ), '1.0.0', true ); ?>  
 ...
 * Start - Cookie Policy
 */
function seon_cookie_policy() {
    $cookie_policy = seon()->get_theme_opt('cookie_policy', 'hide');
    $cookie_policy_description = seon()->get_theme_opt('cookie_policy_description');
    $cookie_policy_btntext = seon()->get_theme_opt('cookie_policy_btntext');
    $cookie_policy_link = get_permalink(seon()->get_theme_opt('cookie_policy_link')); 
 ...
 */
function seon_subscribe_popup() {
    $subscribe = seon()->get_theme_opt('subscribe', 'hide');
    $subscribe_layout = seon()->get_theme_opt('subscribe_layout');
    $popup_effect = seon()->get_theme_opt('popup_effect', 'fade');
    $args = [
 ...
 */
add_action( 'show_user_profile', 'seon_user_fields' );
add_action( 'edit_user_profile', 'seon_user_fields' );
function seon_user_fields($user){
    ...
    ?>
    <h3><?php esc_html_e('Theme Custom', 'seon'); ?></h3>
    <table class="form-table">
        <tr>
            <th><label for="user_name"><?php esc_html_e('Author Name', 'seon'); ?></label></th>
            <td>
        ...
        <tr>
            <th><label for="user_position"><?php esc_html_e('Author Position', 'seon'); ?></label></th>
            <td>
...
add_action( 'personal_options_update', 'seon_save_user_custom_fields' );
add_action( 'edit_user_profile_update', 'seon_save_user_custom_fields' );
function seon_save_user_custom_fields( $user_id )
{
...
function seon_get_user_position() {
